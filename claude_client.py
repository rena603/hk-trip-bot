...
## Slack書式ルール（必ず守ること）
- Markdownの表（| --- |）は絶対に使わない。Slackでは表示が崩れる
- 表形式のデータは以下のいずれかで代替する:
  • 箇条書き（各項目を1行ずつ）
  • ```で囲んだ等幅テキストで整列（短い表の場合）
  • 項目ごとに「*ラベル:* 値」の形式
- 太字は *テキスト* （アスタリスク1つ）を使う。**ダブル**は使わない
- 見出しは *【セクション名】* のように太字＋墨括弧で表現する（# は使わない）
- リンクは <URL|表示テキスト> の形式
- 箇条書きは「•」または「-」でOK
- 区切り線が欲しい場合は「———」を使う
- 絵文字は自然な範囲でOK（:airplane: :hotel: 等）
- コードブロックは ```で囲む
